...
import cv2
import time
from darknet_yolo.p_utils.parse_config import *
from darknet_yolo.p_utils.utils import build_targets, to_cpu
...
class Darknet(nn.Module):
    ...
    def __init__(self):
        ...
        self.module_defs = parse_model_config("darknet_yolo/cfg/yolov3.cfg")
        self.net_info, self.module_list = create_modules(self.module_defs)
        self.yolo_layers = [layer[0] for layer in self.module_list if hasattr(layer[0], "metrics")]
        self.img_size = 288
        self.seen = 0
        self.header_info = np.array([0, 0, 0, self.seen, 0], dtype=np.int32)
        self.path_to_labels = 'darknet_yolo/p_utils/coco.names'
        self.load_darknet_weights("darknet_yolo/weights/yolov3.weights")
    ...
    def yolo_detection(self, frame=cv2.imread("paintings_db/064.png", cv2.IMREAD_COLOR)):
        ...
        start = time.time()
        detect.bounding_box_yolo(frame, self.img_size, self)
        end = time.time()
